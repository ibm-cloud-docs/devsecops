...
Zed Attack Proxy (ZAP) is a free and open source penetration testing (PEN) tool that is maintained under the umbrella of OWASP. At its core, ZAP is a "man-in-the-middle" proxy that sits between the tester's web browser and a web server, intercepting packets, modifying the content if needed, and forwarding those packets to the web server.
{: shortdesc}
