...
1. View the [Getting started with DevSecOps in {{site.data.keyword.cloud_notm}} - Part 1](https://video.ibm.com/embed/recorded/130714354) video.
...
![DevSecOps Continuous Integration toolchain welcome page](images/devsecops-zos-ci-welcome-page.png){: caption="Figure 1. DevSecOps Continuous Integration toolchain welcome page" caption-side="bottom"}
...
{: #zos-ci-toolchain-create-options}
{: step}
...
* From the [{{site.data.keyword.cloud_notm}} console](https://cloud.ibm.com/), click **Menu** ![Menu icon](../icons/icon_hamburger.svg) and select **DevOps**. On the Toolchains page, click **Create toolchain**. On the Create a Toolchain page, click **CI - Develop a secure z/OS app with DevSecOps practices**.

## Set up the CI toolchain name and region
{: #zos-ci-toolchain-name-region}
{: step}
...
{: #zos-ci-toolchain-tool-integrations}
{: step}
...
### Evidence Storage
{: #zos-ci-toolchain-evidence-storage}
...
### Deploy
{: #zos-ci-toolchain-deploy}

Configure the z/OS target where the application is deployed:
...
* **z/OS deployment target type**
   You can build the app by using a standard z/OS target or an air-gapped z/OS target with Bastion Host.

   Enter the IP address or hostname of the target z/OS host, the target z/OS SSH port, and the z/OS user ID.
   
   The IP address or hostname must be visible from the internet. If not, you can use a [Private Worker](https://cloud.ibm.com/docs/ContinuousDelivery?topic=ContinuousDelivery-install-private-workers&interface=ui).
   {: note}

   If you choose an air-gapped z/OS target with Bastion Host, then select a Bastion host Virtual Server Instance from the dropdown list. Then, enter the targeted bastion host SSH port and the user ID. 
   
* **z/OS SSH secret name**
   Enter the z/OS SSH secret name that is declared in your secret manager.

* **DBB High Level Qualifier (HLQ)**
   Optionally, you can enter the DBB Web Application URL. If you do not provide the URL, a full build will be done. If the DBB WebApp URL is provided, an impact build will be done.


### DevOps Insights
{: #zos-ci-toolchain-insights}
...
### Optional tools
{: #zos-ci-toolchain-optional-tools}

#### Slack
{: #zos-ci-toolchain-slack}
...
{: #zos-ci-toolchain-pr-pipeline-run}
...
### Run the CI pipeline
{: #zos-ci-toolchain-ci-pipeline-run}
...
{: #zos-ci-toolchain-pipeline-config}
